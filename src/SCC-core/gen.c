...
//2014-7-9 添加，为了使得语法文件输出更加直观
#define _FILE_MOVE_POS               do{int p=pos;while(p){fprintf(parseFile," ");p--;}}while(0)
// 由于前面_MOVE_NEXT_POS 已经修改了pos，所以这里直接移到位置就行了。
//2014-7-9 添加，为了使得语法文件输出更加直观
...
#define Out_parseBeginMark()         do{Format_setColor(COLOR_PARSE);_MOVE_NEXT_POS;printf("<%s>{\n",__FUNCTION__);_FILE_MOVE_POS;fprintf(parseFile,"<%s>{\n",__FUNCTION__);}while(0)
#define Out_parseEndMark()           do{Format_setColor(COLOR_PARSE);printf("<%s>}\n",__FUNCTION__);_FILE_MOVE_POS;fprintf(parseFile,"<%s>}\n",__FUNCTION__);_MOVE_BACK_POS;}while(0)
...
// 错误处理不会影响词法分析，词法分析模块独立，如果词法分析结果出错，不可能是后级造成
