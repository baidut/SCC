...
// 采用fscanf可以自动过滤空格，但是不能计量行号，故不采用
...
void Lex_dispToken(Token* tk){
  //printf("\r");
  printf("%10s",tokenName[(unsigned int)(tk->type)]);
  printf(" ");// 用tab不整齐，改为空格间隔
  switch(tk->type){
    case TK_IDENT:
    case TK_RELOP:
    case TK_NUM:
...
void Lex_writeToken(Token* tk,FILE* fout){
  //ASSERT(fout!=NULL);
  fprintf(fout,"%10s",tokenName[(unsigned int)(tk->type)]);
  fprintf(fout," ");// 用tab不整齐，改为空格间隔
  switch(tk->type){
    case TK_IDENT:
    case TK_RELOP:
    case TK_NUM:
    case TK_UNDEF:
      fprintf(fout,"\"%s\"",tk->value);
      break;
    default:
      fprintf(fout,"_");break;
  }
  fprintf(fout,"\n");
